...
- **Single shared K8s scheme + dual client.** `internal/k8s.Scheme` is a package-level `runtime.Scheme` initialized at package init; every package that needs typed K8s access imports `internal/k8s` rather than constructing its own scheme. `k8s.NewClients(cfg)` returns both a controller-runtime typed `client.Client` (for SSA) and a `kubernetes.Interface` (for `tools/watch.UntilWithSync`'s `cache.ListWatch`) built from one shared `*rest.Config` — callers can't accidentally pick up one client and skip the other. Startup-time sanity check: `cfg.CR.APIGroup` must equal `wizapi.GroupVersion.Group`, otherwise typed Patch silently sends to the imported group and never hits the operator.
