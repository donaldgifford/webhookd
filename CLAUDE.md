...
`webhookd` is a Go webhook receiver. IMPL-0001 Phases 0–4 are complete: bootstrap, configuration (`internal/config`), observability (`internal/observability`), HTTP framework (`internal/httpx`), and the webhook handler with signature verification (`internal/webhook/{signature,handler}.go`). The webhook package ships HMAC-SHA256 verification over a `v0:<timestamp>:<body>` canonical message, replay protection via `VerifyTimestamp` against `cfg.TimestampSkew`, and a per-provider HTTP handler that emits the canonical metrics from DESIGN-0001 §Metrics with the right labels for every error class. Coverage on `internal/webhook` is 94.7%; `FuzzSignatureVerify` runs cleanly at 1M execs/sec. `make ci` is green. Active work tracked in `docs/impl/0001-phase-1-stateless-receiver-implementation.md`.
