...
`webhookd` is a Go webhook receiver. IMPL-0001 Phases 0–5 are complete: bootstrap, configuration (`internal/config`), observability (`internal/observability`), HTTP framework (`internal/httpx`), webhook handler with HMAC-SHA256 + replay protection (`internal/webhook`), and full application wiring (`cmd/webhookd/main.go`). The entry point performs the Walk1.md §2 five-phase startup, dispatches both servers in goroutines, flips an `atomic.Bool` ready, and waits for either a listener error or `signal.NotifyContext` (SIGTERM/SIGINT) before draining via `cfg.ShutdownTimeout`. Tracer-provider and server shutdown both use detached background contexts so the drain budget survives signal cancellation. Integration tests in `cmd/webhookd/main_test.go` use `goleak.VerifyTestMain` to catch goroutine leaks and exercise a full sign→post→scrape round-trip against real ports. `make ci` is green. Phase 6 (operational extras: pprof, rate limiting, license headers) is the only remaining work in IMPL-0001.
