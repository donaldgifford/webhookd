...
**Active branch:** Phase 2 work on `feat/design-0002-impl` (DESIGN-0002: JSM webhook → `SAMLGroupMapping` CR provisioning). **IMPL-0002 is drafted with all 12 open questions resolved** — the doc at `docs/impl/0002-phase-2-jsm-to-samlmapping-provisioning-implementation.md` has a Resolved Decisions section mirroring IMPL-0001's pattern. **Phases 0–2 complete.** Phase 0: controller-runtime / apimachinery deps, `make tools-envtest`, `internal/webhook/wizapi/` stub. Phase 1: `JSMConfig`, `CRConfig`, `EnabledProviders`, validation. Phase 2 (Provider Interface & Action Union): `internal/webhook/provider.go` (`Provider` interface), `action.go` (`Action` typed union with `NoopAction` + `ApplySAMLGroupMapping`, plus `ErrBadRequest` / `ErrUnprocessable` sentinels), `result.go` (`ExecResult` + `ResultKind` + `HTTPStatus()` mapping), `providertest/mock.go` (configurable fake). The Phase 1 webhook handler (`internal/webhook/handler.go` + `handler_test.go`) is **deleted**; `cmd/webhookd/main.go` holds a 503 tombstone with `Retry-After: 30` at `POST /webhook/{provider}` until Phase 6 wires the dispatcher. The integration test in `cmd/webhookd/main_test.go` asserts the tombstone behavior + middleware-chain liveness (request-id propagation). Phase 3 (K8s Client & Scheme) is the next unchecked task. **Important:** during the open-question review, the CRD shape diverged substantially from DESIGN-0002's strawman. The canonical shape now lives in `docs/examples/samples/wiz_v1alpha1_samlgroupmapping.yaml` (and friends `*_project.yaml`, `*_userrole.yaml`); DESIGN-0002 still references the old `SAMLMapping`/`wiz.example.com`/`team`+`projects[]` shape and will be updated in Phase 8 when its status flips to Implemented. **Read IMPL-0002's Resolved Decisions before reading DESIGN-0002 — IMPL-0002 supersedes the design doc on shape questions until DESIGN-0002 is rewritten.**
